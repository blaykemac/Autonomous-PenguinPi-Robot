...
class EKF:
    ...
    def __init__(self, robot, true_markers):
        # State components
        self.robot = robot
        #self.markers = np.zeros((2,0))
        self.markers = true_markers
        self.taglist = []
        self.taglist = [tag for tag in range(1, 11)] # manually set tags since we arent adding landmarks
        

        # Covariance matrix
        #self.P = np.zeros((3,3)) # removed this to allow for true landmarks immediately
        self.P = np.zeros((self.number_landmarks() * 2 + 3, self.number_landmarks() * 2 + 3))
    ...
    def predict(self, raw_drive_meas):
        ...
        #print(f"no landmarks: {self.number_landmarks()}")
